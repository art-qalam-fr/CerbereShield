...
REM Appliquer le PLAN DE LIBÉRATION DE PORTS généré par Cerbere Security Shield
REM Le script vit dans scripts\ : on remonte à la racine du projet

cd /d "%~dp0.."

echo === APPLICATION DU PLAN DE LIBERATION ===
powershell -NoProfile -ExecutionPolicy Bypass -File ".\scripts\powershell\security_hardening.ps1" -Action "release"
set RC=%errorlevel%

if %RC% equ 0 (
    set "NOTIF_MSG=Liberation des ports effectuee."
) else (
